...
def passed(msg):
    print(f"passed test: {msg}")

def get_equiprobable_policy_actions(state):
    return [("up", .25), ("down", .25), ("left", .25), ("right", .25)]
...
def find_exact_diffs(v, vexpect):
    ...
    for i in range(len(vexpect)):
        ...
        diff = val not in ve if isinstance(ve, tuple) else (val != ve)
        if diff:      
...
def policy_eval_core_test(eval_func, variant):
    print()
    print(f"Testing: Policy Evaluation ({variant})")
    ...
    if (len(v) != len(vexpect)):
        error(f"length of  list is neq {len(vexpect)}")
        return
    passed(f"length of list = {len(vexpect)}")
    ...
    if diffs > 0:
        error(
            f"list elements don't match expected values: # of mismatches={str(diffs)}"
        )
        return
    ...
    pass_code = f"{pass_code[:4]}-{pass_code[4:]}"
    print(f"PASSED: Policy Evaluation ({variant}) passcode = {pass_code}")
...
def policy_iteration_core_test(eval_func, name, passcode_index):
    ...
    if (not isinstance(v, list)) or (len(v) != state_count):
        error(f"v is not a list of length={state_count}")
        return
    passed(f"v is list of length={state_count}")
    ...
    if diffs > 0:
        error(f"v elements don't match expected values: # of mismatches={str(diffs)}")
        return
    passed("values of v elements")

    # 5. check type/length of pi
    if (not isinstance(pi, list)) or (len(pi) != state_count):
        error(f"pi is not a list of length={state_count}")
        return
    passed(f"pi is list of length={state_count}")
    ...
    if diffs > 0:
        error(f"pi elements don't match expected values: # of mismatches={str(diffs)}")
        return
    ...
    pass_code = f"{pass_code[:4]}-{pass_code[4:]}"
    print(f"PASSED: {name} passcode = {pass_code}")
